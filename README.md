...
- Mock-сценарии `success/loading/empty/error` влияют только на исторические локальные `/api/*` запросы; реальные `/api-web/*` и `/api-mobile/*` не перехватываются.

## Целевая таблица

Компонент другого разработчика находится напрямую в `src/Components/Adressbook` и подключается с исходной точки входа `AdressBook`. UI-логика компонента сохранена; цепочка `AdressBook → People → useGetPeople` минимально переведена с глобальных hooks на props.

Переносимая интеграция целиком находится в двух папках:

- `src/apps/AddressBook/pages/ContactsPage` сохраняет загрузку и состояния страницы;
- `src/apps/AddressBook/pages/ContactsPage` передаёт исходный `PERSONADDRESSBOOK.data.content` без повторной нормализации;
- `src/apps/AddressBook/components/ImportedAdressbook` только загружает донорский компонент и передаёт ему raw people, loading и выбранного сотрудника;
- `src/apps/AddressBook/mocks/installFetchMock.ts` сохраняет только исторические PoC-моки `/api/*` и не перехватывает реальные API таблицы;
- `src/Components/Adressbook` содержит сам компонент, его тип входных данных и шесть SVG, отсутствовавших в текстовой выгрузке;
- локальная Vite-совместимость для SVG рендерит содержимое каждого файла, не заменяя иконки общей заглушкой.

Новых корневых `context`, `hooks`, `host` или `vendor` для таблицы нет. При обновлении компонента нужно сохранить внешний prop-контракт `people/isLoading`, начальное раскрытие и два опциональных prop избранного либо минимально повторить эту цепочку после замены папки.

Тема не адаптируется внутри интеграции: компонент использует существующий `ThemeProvider` и полный theme-контракт целевого окружения.

Точка входа загружается eager-вызовом `import.meta.glob` внутри адаптера. Это не отдельный runtime-модуль: Vite включает компонент в основной bundle синхронно. Такой импорт не втягивает всё донорское дерево в strict-проверку приложения и позволяет не исправлять массово его исходную типизацию.
