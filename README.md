...
- Mock-сценарии `success/loading/empty/error` влияют на локальные `/api/*` запросы и точный mock `multiSearch`; остальные `/api-web/*` идут во внешний контур.
...
- `src/apps/AddressBook/components/ImportedAdressbook` преобразует `Employee[]` в ожидаемую таблицей форму и передаёт выбранного сотрудника для начального раскрытия;
- `src/apps/AddressBook/mocks/installFetchMock.ts` содержит точные локальные mocks поиска и профиля;
- `src/Components/Adressbook` содержит сам компонент, его тип входных данных и недостающие SVG.

Новых корневых `context`, `hooks`, `host` или `vendor` для таблицы нет. При обновлении компонента нужно сохранить его внешний prop-контракт `people/isLoading` либо минимально повторить эти три изменения после замены папки.
