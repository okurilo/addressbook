...
- После интеграционного аудита избранное больше не кэширует завершённый запрос и заново загружает custom-группу «Избранные» при каждом входе на экран; верхний поиск снова имеет собственный dropdown, независимое объединение людей и подразделений и очистку `q`; справочные телефоны сохраняют глобальный фильтр при смене категории.
...
- Dropdown поиска больше не вызывает `groups`: один `multiSearch` нормализует `PERSONADDRESSBOOK` и плоский `ORGSTRUCTURE` (`id/fullName/typeName/layer/layerName`), а выбор структуры очищает поиск и открывает узел, сотрудники которого запрашиваются с `orgFilter`.
