...
- Раздел «Все контакты» без `q` загружает четыре недавние записи через реальный `GET globalsearch/api/v3/history?paths=all&size=4`; результат `http.get<SearchHistoryItem[]>` является массивом без повторного чтения `data`.
