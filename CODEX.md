...
- Dropdown на вкладке кадровой структуры запрашивает только `ORGSTRUCTURE`, но Enter по-прежнему запускает глобальный поиск людей; структурные подсказки получают подпись «Найдено в кадровой структуре».
...
- Списки людей используют страницу размером 20 и query-параметр `page`; `multiSearch` передаёт серверные page-метаданные, а custom-группы безопасно дочитываются постранично даже без метаданных.
