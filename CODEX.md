...
- Избранное загружается из кастомной группы: `GET srv/v7/people/teams` находит запись с `isCustom=true` и `name=Избранные`, после чего `GET srv/v7/people/teams/{id}?page=0&size=20&isCustom=true` загружает участников. Внешнее транспортное `data` снимает общий `http`.
