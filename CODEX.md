...
- Кадровая структура загружается через `GET /api-web/posts/api/v1/addressbook/groups`: корневой запрос не содержит `id`, а выбранный узел передаёт только строковый UUID в `id`; объект узла никогда не сериализуется в query.
- Каждый экран структуры запускает один `groups` и один `multiSearch`, отменяет устаревшие запросы через общий `AbortSignal` и не держит в зависимостях эффекта изменяемые им кэши.
- Пустая строка внутри выбранного подразделения передаёт его UUID как `orgFilter`; непустой поиск людей всегда глобальный и не содержит `orgFilter`, даже если запущен со страницы подразделения.
...
- Фильтрация списка подразделений при поиске людей отмечена как критичный открытый продуктовый вопрос; реализация без отдельного согласования запрещена.
...
- Пока источник подразделений для результатов глобального поиска людей в кадровой не согласован, sidebar не подменяет его обычными children текущей структуры.
