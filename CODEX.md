...
- В целевом компоненте минимально изменена цепочка получения данных `AdressBook → People → useGetPeople`: готовый массив приходит через props, а `createRowData`, таблица и строки сохраняют исходную логику.
- Чтобы замена таблицы не ломала существующее избранное, по той же цепочке опционально передаются `favoritePersonIds/onToggleFavorite`; `Profile/Header` только связывают уже существующую кнопку-звезду с callback приложения.
- `ContactsPage` передаёт таблице исходный `PERSONADDRESSBOOK.data.content` без промежуточной нормализации в `Employee` и обратного восстановления донорской формы.
- `src/apps/AddressBook/components/ImportedAdressbook` не содержит API-адаптации: это только типизированная runtime-граница загрузки донорского компонента; тема приходит напрямую из целевого окружения.
...
- Реальные ручки `multiSearch` и профиля не подменяются внутри `src/apps/AddressBook`; существующий PoC fetch-mock оставлен в исходном объёме только для исторических `/api/*` сценариев приложения.
...
- Локальный `hostTheme` повторяет уже существующий контракт целевого окружения для таблицы (`mode`, `tokens.current.colors.grey.solid`, `tokens.current.core.link`); эти поля не добавляются provider-адаптером внутри переносимых папок.
- Локальный Vite proxy направляет как `/api-web`, так и используемый профилем `/api-mobile` в существующий dev-контур без mock-ответов.
- `ProfileImage` восстановлен по исходной выгрузке с импортами `getIconUrl` и общего `StatusIcon`; отсутствующие в песочнице зависимости замоканы вне двух переносимых папок и не входят в контракт копирования.
- Текстовая выгрузка коллеги не содержала файлов `Profile/assets/*.svg`. В репозитории сохранены шесть различимых временных SVG, а локальный Vite compatibility-плагин теперь рендерит фактическое содержимое каждого файла вместо одинакового круга. При получении оригинальных assets их можно заменить без изменений кода компонента и конфигурации целевого окружения.
