...
- Восстановленная интеграция использует публичный `AdressBook`, сохраняет исходную цепочку `People/useGetPeople/utils/Table/Profile`, не содержит `@ts-nocheck`, а локальные моки включаются только host-флагом `VITE_ADDRESSBOOK_USE_MOCKS=true`.
