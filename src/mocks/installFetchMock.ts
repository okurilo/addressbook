...
const getSuccessPayload = (url: URL): unknown => {
  ...
  if (url.pathname.startsWith('/api/directory/departments/')) {
    const departmentPath = decodeURIComponent(url.pathname.replace('/api/directory/departments/', ''));
...
export const installFetchMock = (): void => {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url.pathname.startsWith('/api/directory/departments/') && url.pathname !== '/api/directory/departments/root') {
      const departmentPath = decodeURIComponent(url.pathname.replace('/api/directory/departments/', ''));
