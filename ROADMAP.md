...
- [x] Разобрать `PERSONADDRESSBOOK.data.content` после извлечения внешнего `data` общим запросником, без `any`.
...
### Итерация 27. Сотрудники в кадровой структуре через multiSearch

- [x] Заменить mock-карточки подразделений на корневом экране кадровой структуры таблицей сотрудников.
...
### Итерация 30. Проверка контракта multiSearch по фактическому ответу

- [x] Вынести нормализацию ответа в чистую функцию.
- [x] Добавить тест на структуру `PERSONADDRESSBOOK` из предоставленного примера.
- [x] Проверить ФИО, UUID, подразделение, должность, email, статус, адрес и количество.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 31. Уточнение контракта результата http.get

- [x] Зафиксировать, что общий `http.get` извлекает внешнее поле `data` сетевого ответа.
- [x] Читать `PERSONADDRESSBOOK` непосредственно из результата `getSearchData`.
- [x] Удалить рекурсивный поиск транспортных обёрток и альтернативные схемы полей.
- [x] Синхронизировать локальный host-stub с поведением запросника контура.
- [x] Прогнать тест нормализации подтверждённого ответа.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 32. Единое правило разбора ответов http.*

- [x] Распространить контракт извлечения внешнего `data` на все методы `http.*`.
- [x] Зафиксировать, что generic описывает внутренний результат, а не сетевой envelope.
- [x] Запретить повторное чтение `response.data` и неподтверждённые fallback-обёртки.
- [x] Зафиксировать сохранение вложенных бизнес-полей `data`.
- [x] Обязать локальные host-stubs повторять контракт для каждого реализованного метода.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 33. Фиксация сценариев верхнего поиска

- [x] Разделить поиск людей по Enter и выбор подсказки из выпадающего списка.
- [x] Зафиксировать сохранение текущей вкладки при поиске людей по Enter.
- [x] Зафиксировать сброс `orgFilter` при поиске людей внутри выбранного подразделения.
- [x] Зафиксировать переход во «Все контакты» и показ одной карточки при выборе человека.
- [x] Зафиксировать переход в выбранный узел кадровой структуры при выборе подразделения.
- [x] Отметить части сценариев, ожидающие API организационной структуры.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 34. Поиск людей: Enter и выпадающий список

- [x] Не запускать основной поиск и не менять вкладку при каждом вводе символа.
- [x] Загружать подсказки людей через `multiSearch` без `orgFilter`.
- [x] По Enter оставаться в текущем разделе «Все контакты» или «Кадровая структура».
- [x] При Enter внутри подразделения переходить на корень кадровой структуры и не передавать `orgFilter`.
- [x] При выборе человека переходить во «Все контакты» и показывать только его раскрытую карточку.
- [x] Оставить подсказки и переходы по оргструктурам до подключения их API.
- [x] Удалить ранее отслеживаемый `dist/index.html` из Git; весь `/dist/` остаётся в `.gitignore`.
- [x] Прогнать точечный тест контракта `multiSearch`.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 35. Ограничение частоты поисковых запросов

- [x] Задать явный debounce 500 мс для запросов подсказок `multiSearch`.
- [x] Сохранить отмену устаревшего запроса через `AbortController`.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 36. Реальная история и единый режим результатов поиска

- [x] Добавить типы и продуктовые функции API истории внутри `src/apps/AddressBook`.
- [x] Загружать четыре недавние записи через `GET history?paths=all&size=4`.
- [x] Вызывать `selectSearchHistory` при выборе недавней записи.
- [x] Заменить mock-недавние на список реальной истории во «Все контакты».
- [x] Показывать заголовок «Результаты поиска» при поиске людей в кадровой структуре.
- [x] Показывать слева уникальные подразделения найденных сотрудников.
- [x] Добавить тест URL, разбора ответа и выбора записи истории.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 37. Только люди в истории контактов

- [x] Заменить `paths=all` на `paths=persons` при загрузке недавних.
- [x] Отбрасывать известные неперсональные контексты из ответа history.
- [x] Проверить тестом, что курс не попадает в результат истории.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 38. Сохранение поиска при смене вкладки

- [x] Сохранять `q` при переходе «Все контакты» → «Кадровая структура».
- [x] Сохранять `q` при обратном переходе в «Все контакты».
- [x] Не переносить `personId` выбранной карточки между вкладками.
- [x] Не переносить поиск людей в остальные разделы справочника.
- [x] Добавить тест построения маршрутов навигации.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 39. Глобальное состояние поискового фильтра

- [x] Сохранять `q` при любых переходах между разделами.
- [x] По Enter оставлять пользователя в инициировавшем поиск разделе.
- [x] Применять `q` к таблицам избранного и справочных телефонов.
- [x] Сохранять `q` при переключении категорий справочных телефонов.
- [x] При выборе человека очищать строку и удалять глобальный `q`.
- [x] Загружать выбранного человека по `personId` и внутреннему `personQuery`.
- [x] Зафиксировать такое же завершение поиска для будущего выбора оргструктуры.
- [x] Расширить тесты маршрутизации глобального фильтра.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 40. Избранное из кастомной группы

- [x] Загрузить все группы человека через `GET people/teams`.
- [x] Найти кастомную группу с именем `Избранное`.
- [x] Загрузить первую страницу участников через `GET people/teams/{id}`.
- [x] Нормализовать ФИО и контакты участников в модель `Employee`.
- [x] Показать участников группы на странице «Избранное».
- [x] Добавить тесты поиска группы и нормализации участника.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 41. Сохранение человека в избранное

- [x] Добавить человека в существующую группу через `POST people/custom-groups/update`.
- [x] Создать группу `избранное` с первым человеком через `POST people/custom-groups`, если группы нет.
- [x] Перевести удаление из избранного на `personsToDelete` того же update-контракта.
- [x] Обновлять локальное состояние звезды после успешного POST, чтобы исключить гонку с перечитыванием группы.
- [x] Добавить тесты создания, добавления и удаления.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 42. Реальная кадровая структура

- [x] Добавить типизированный запрос `GET addressbook/groups` через общий `http`.
- [x] При входе без `id` загружать оргединицу текущего пользователя.
- [x] При выборе узла передавать его `id` в `addressbook/groups`.
- [x] Догружать названия дочерних узлов по UUID из `children`.
- [x] Показывать слева кнопку «Наверх», выбранный узел и его дочерние узлы.
- [x] Загружать сотрудников выбранного узла через `multiSearch` с `orgFilter=id`.
- [x] Сохранить глобальный поиск людей без `orgFilter`.
- [x] Прогнать точечные тесты URL API.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 43. Встраивание целевого компонента в границе двух папок

- [x] Сохранить `src/apps/AddressBook` и `src/Components/Adressbook` единственной переносимой границей.
- [x] Удалить vendor-пакет, manifest, отдельный harness, корневые search-context/hooks и host-компонент подключения.
- [x] Подключить исходный `AdressBook`/`People`, а не только `common/Table`.
- [x] Минимально перевести `AdressBook → People → useGetPeople` на входные props без изменения `createRowData` и UI-логики.
- [x] Передавать совместимый raw-контракт `PERSONADDRESSBOOK.data.content` напрямую, без цикла `raw → Employee → raw`.
- [x] Ограничить TypeScript-контракт компонента в адаптере, не исправляя массово strict-ошибки донорских файлов.
- [x] Использовать тему целевого окружения напрямую, без дополнительного provider и fallback-токенов.
- [x] Добавить недостающие assets внутрь папки компонента и не подменять совместимые API поиска/профиля внутри переносимых папок.
- [x] Сохранить сценарии loading, error, empty, history и success существующей страницы.
- [x] Сохранить автоматическое раскрытие карточки сотрудника, выбранного из поисковой подсказки.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.
- [x] Проверить строку таблицы, раскрытие профиля и контакты в браузере.
- [x] Дополнить только локальный host-theme и proxy контрактами, уже существующими в целевом окружении.

### Итерация 44. Исправления по ревью PR #6

- [x] Удалить перехват совместимых `/api-web` и `/api-mobile` из переносимого fetch-mock.
- [x] Передавать таблице исходный `PERSONADDRESSBOOK.data.content` без повторной нормализации.
- [x] Сохранить старый сценарий добавления и удаления сотрудника из избранного через кнопку-звезду профиля.
- [x] Восстановить исходный `ProfileImage`, а отсутствующие зависимости замокать только в локальном окружении.
- [x] Заменить одинаковую SVG-заглушку на совместимый loader фактического содержимого шести разных assets.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.
