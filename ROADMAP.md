...
- [x] Добавить подпись структурных результатов.
- [x] Перенести контент под вкладками во внутренний scroll-контейнер.
- [x] Добавить действия «Написать всем» и «Поставить встречу».
- [x] Добавить накопительную дозагрузку «Показать ещё» для результатов поиска, подразделений и custom-групп.
- [x] Ограничить dropdown одной категорией активной вкладки: контакты — `PERSONADDRESSBOOK`, кадровая — `ORGSTRUCTURE`.
- [x] Сохранить независимые Enter-flow и переходы по выбранным подсказкам.
- [x] Провести повторный аудит бизнес-правил: убрать неподтверждённые структуры из результатов поиска и перевести custom-группы на фактическую постраничную дозагрузку.
- [ ] Прогнать `npm run typecheck` — блокируется ранее перенесёнными несовместимостями `src/Components/Adressbook/**`.
- [ ] Прогнать `npm run build` — останавливается на тех же TypeScript-ошибках до запуска Vite.
...
### Итерация 27. Импорт внешнего компонента Adressbook

- [x] Дословно перенести 69 файлов контекстного снимка в `src/Components/Adressbook`.
- [x] Не подключать, не адаптировать и не изменять импортированный код до следующих указаний.

### Итерация 28. Интеграция внешней таблицы и Profile

- [x] Заменить общую таблицу сотрудников на импортированный generic `Table`.
- [x] Адаптировать `Employee[]` к строкам с `pid` и `_profile` без изменения `renderExpanded`.
- [x] Сохранить телефон, email и избранное в `ConnectCell`.
- [x] Подключить полный Profile, `/api-mobile` и teams API через общий HTTP-слой.
- [x] Добавить SVG-заглушку, Pulse-stubs, theme-ветки и внешние зависимости.
- [x] Проверить контакты, избранное, подразделение и раскрытие Profile в браузере.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 29. Реальная кадровая структура и глобальный поиск

- [x] Подключить `GET /api-web/posts/api/v1/addressbook/groups` через общий HTTP-клиент.
- [x] Разделить корневой запрос без `id` и открытие узла со строковым UUID.
- [x] Устранить запросы `id=[object Object]` и повторные вызовы из реактивных кэшей.
- [x] Отменять устаревшие запросы структуры и поиска через `AbortSignal`.
- [x] Передавать `orgFilter` только для выбранного подразделения при пустой строке.
- [x] Сохранять глобальную строку между вкладками и применять её по Enter без переключения вкладки.
- [x] Подключить существующие suggestions к переходам в сотрудника и подразделение.
- [x] Сохранить экран «недавние» при пустом поиске во «всех контактах».
- [ ] Проверить структуру, dropdown и количество запросов в целевом host-окружении.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 30. Самодостаточная граница поставки AddressBook

- [x] Перенести поиск и оргструктуру в `src/apps/AddressBook/api/directory`.
- [x] Перенести Profile API в `src/Components/Adressbook/api`.
- [x] Оставить снаружи только общий `src/http-requests/http.ts`.
- [x] Обновить все импорты внутри поставляемых каталогов.
- [x] Зафиксировать запрет feature-specific API-файлов за пределами AddressBook.
- [x] Проверить отсутствие внешних feature-импортов.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 30. Единый источник бизнес-логики

- [x] Собрать подтверждённые сценарии поиска, структуры, истории и избранного в `docs/BUSINESS-RULES.md`.
- [x] Зафиксировать контракты HTTP и известный состав API.
- [x] Обязать Codex читать бизнес-правила до планирования и вопросов.
- [x] Обязать Codex обновлять документ после каждого бизнес-уточнения.
- [x] Добавить ссылку на бизнес-правила в README и `CODEX.md`.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 31. Восстановление реальных сценариев AddressBook

- [x] Убрать повторное раскрытие `response.data` в `multiSearch`, `groups` и Profile.
- [x] Привести локальный `HttpRequest` к целевой семантике однократного unwrap.
- [x] Подключить «недавние» к `globalsearch/api/v3/history` вместо mock API.
- [x] Подключить «Избранное» к `srv/v7/people/teams` и загрузке участников custom-группы.
- [x] Удалить mock endpoints из рабочих потоков недавних и избранного.
- [x] Зафиксировать границу поставки и семантику HTTP-оболочки.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.
...
### Итерация 33. Восстановление экранов после интеграции

- [x] Убрать долговременный кэш избранного и исправить имя custom-группы «Избранные».
- [x] Повторно загружать участников при каждом входе на экран избранного.
- [x] Вернуть собственный dropdown поиска и независимую обработку людей и подразделений.
- [x] Добавить очистку строки вместе с параметром `q`.
- [x] Вернуть фильтрацию справочных телефонов и сохранение query при смене категории.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 34. Локальный адаптер Empty/Page
...
### Итерация 36. Иконка избранного

- [x] Убрать двойную рамку вокруг звезды в верхней кнопке избранного.
- [x] Добавить компактную акцентную подложку и заливку активного состояния.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 37. Полный контракт новой таблицы

- [x] Подтвердить использование `src/Components/Adressbook` во «Всех контактах» и «Избранном».
- [x] Сохранить в API-модели доступные ФИО, фото, `funcBlock`, контакты и отсутствие.
- [x] Передать полный доступный контракт `AdressbookPerson` через общий адаптер таблицы.
- [x] Передать те же дополнительные данные в отдельный экран Profile.
- [x] Не изменять `src/Components/Adressbook/**`.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 38. Минимизация адаптера таблицы

- [x] Сверить передаваемые поля с фактическими чтениями таблицы и Profile.
- [x] Удалить неиспользуемое дублирование внутренней и внешней почты.
...
### Итерация 40. Локальный host-mock закрытого контура

- [x] Вынести активный mock-транспорт в `src/host/**` без изменений поставляемых каталогов.
- [x] Включить локальные host-моки по умолчанию.
- [x] Оставить реальный proxy за явным `VITE_ADDRESSBOOK_USE_REAL_API=true`.
- [x] Эмулировать `multiSearch`, history, teams/custom-groups, groups и Profile API.
- [x] Согласовать поиск, историю, группы и Profile через единый набор host-fixtures.
- [x] Синхронизировать варианты имени «Избранные»/«Избранное» через единый host group id.
- [x] Сохранять изменяемые custom-группы и историю в `localStorage`.
- [x] Сохранить сценарии `success`, `loading`, `empty`, `error`.
- [x] Возвращать контролируемый `404` для неизвестных API-путей в mock-режиме.
- [x] Проверить строгую компиляцию новых host-файлов.
- [ ] Общий `npm run typecheck` заблокирован существующими ошибками буквального snapshot в защищённых каталогах.
- [ ] Общий `npm run build` заблокирован теми же существующими ошибками буквального snapshot.

### Итерация 41. Буквальный перенос целевых снимков 16 июля 2026

- [x] Перенести 89 файлов `src/Components/Adressbook/**` из `context.md`.
- [x] Перенести 75 файлов `src/apps/AddressBook/**` из объединённых частей снимка без адаптации.
- [x] Удалить файлы внутри двух целевых каталогов, отсутствующие в снимках.
- [x] Проверить побайтовое совпадение всех 164 файлов с переданным целевым вариантом.
- [x] Запустить `npm run typecheck` и зафиксировать несовместимости целевого снимка без исправления.
- [x] Запустить `npm run build` и зафиксировать те же несовместимости без исправления.

### Итерация 42. Диагностика локальной сборки целевого снимка

- [x] Повторно запустить строгую TypeScript-проверку без изменения целевых каталогов.
- [x] Отделить несовместимости локальной host-обвязки от внутренних рассинхронизаций снимка.
- [x] Подтвердить внешние причины: `ES2020` без `String.replaceAll` и отсутствующий Snackbar icon `Error`.
- [x] Подтвердить внутренние причины: route props, `renderActions`, `TablePhone`, `IconButton ref` и Profile-типы.
- [x] Зафиксировать, что полное восстановление strict-сборки только внешней обвязкой невозможно без небезопасной подмены типов или модулей.

### Итерация 43. Восстановление локальной strict-сборки

- [x] Перевести локальный TypeScript target/lib на ES2021 для используемого `String.replaceAll`.
- [x] Дополнить Snackbar-stub используемой целевым снимком иконкой ошибки.
...
### Итерация 45. Стабилизация экрана «Избранное»

- [x] Убрать повторную загрузку всех custom-групп после инициализации общего состояния звёздочек.
- [x] Исключить применение глобального `q` к вкладке «Все» и отдельным custom-группам.
- [x] Сохранить `q` и `groupId` в URL при переключении групп.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.

### Итерация 46. Защита пагинации custom-групп

- [x] Считать пустой `content` конечной страницей независимо от backend-флага `last`.
- [x] Не показывать «Показать ещё» для пустой первой страницы custom-группы.
- [x] Останавливать последующую и полную дозагрузку на пустой странице.
- [x] Прогнать `npm run typecheck`.
- [x] Прогнать `npm run build`.
