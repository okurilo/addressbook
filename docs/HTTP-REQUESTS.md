...
Структура загружается через общий `http`:

- при входе без поискового запроса используется `posts/api/v1/addressbook/groups` без `id`, который возвращает текущую структуру пользователя;
- при непустом `q` и отсутствии выбранного UUID используется `posts/api/v1/addressbook/groups/root`;
...
- один явный переход запускает не более одного запроса структуры, а устаревший запрос отменяется через `AbortSignal`.

Сервер оборачивает ответы в транспортное поле `data`. Поэтому `http.get<GroupNode>()` для `/groups` возвращает уже `GroupNode`, а `http.get<GroupNode[]>()` для `/groups/root` — уже массив корневых узлов. Узел содержит `id`, `type`, `name`, `hasChild`, `children` и опциональный `parentTree`.

При пустом текстовом запросе выбранный `group.id` передаётся в `multiSearch` как `orgFilter`. Непустой поиск без выбранного UUID выполняется по всей компании без `orgFilter`. После выбора структуры строка `q` сохраняется, а её UUID передаётся вместе с `query` в `multiSearch`; последующие страницы используют ту же комбинацию параметров.
